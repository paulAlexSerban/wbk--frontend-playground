...
export const UIController = (el) => {
  ...
  const setupFullName = (currPerson, storageQuestList, admin) => {
    if (global.elements.firstNameInput.value !== "" && global.elements.lastNameInput.value !== "") {
      if (!(global.elements.firstNameInput.value === admin[0] && global.elements.lastNameInput.value === admin[1])) {
    ...
    for (let i = 0; i < userData.getPersonData().length; i++) {
      resultHTML = `
        <p class="person person-${i}">
          <span class="person-${i}">${userData.getPersonData()[i].firstName} ${userData.getPersonData()[i].lastName} - ${userData.getPersonData()[i].score} Points</span>
