...
(() => {
  const PigGame = (el) => {
    ...
    const setupEventListeners = () => {
      ...
      global.elements.btnRoll.addEventListener("click", () => {
        if (global.state.playing) {
          ...
          // 2. Display dice
          global.elements.diceEl.classList.remove("widget-pig-game__hidden");
          global.elements.diceEl.src = `/images/dice-${dice}-original.webp`;

          // 3. Check for rolled 1
          if (dice !== 1) {
            // Add dice to current score
            global.state.currentScore += dice;
            document.getElementById(`current--${global.state.activePlayer}`).textContent = global.state.currentScore;
          } else {
      ...
      global.elements.btnHold.addEventListener("click",  () => {
        if (global.state.playing) {
          ...
          if (global.state.scores[global.state.activePlayer] >= 100) {
            ...
            global.elements.diceEl.classList.add("widget-pig-game__hidden");

            document
              .querySelector(`.js-player--${global.state.activePlayer}`)
              .classList.add("widget-pig-game__player--winner");
            document
              .querySelector(`.js-player--${global.state.activePlayer}`)
              .classList.remove("widget-pig-game__player--active");
          } else {
    ...
    const start = () => {
      ...
      global.elements.player0El.classList.remove("widget-pig-game__player--winner");
      global.elements.player1El.classList.remove("widget-pig-game__player--winner");
      global.elements.player0El.classList.add("widget-pig-game__player--active");
      global.elements.player1El.classList.remove("widget-pig-game__player--active");
    };
    ...
    const switchPlayer = () => {
      ...
      global.elements.player0El.classList.toggle("widget-pig-game__player--active");
      global.elements.player1El.classList.toggle("widget-pig-game__player--active");
    };
