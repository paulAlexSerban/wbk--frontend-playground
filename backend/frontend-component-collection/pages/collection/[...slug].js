...
export default function ComponentPage({ frontmatter, content, cmpStyles, stylesheets }) {
    ...
    return (
        <GenericTemplate title={frontmatter.title} stylesheets={stylesheets}>
            <style jsx>{`
                ${stylesheets[frontmatter.assets["main-css"]]}
            `}</style>
            {frontmatter.assets["main-js"] && (
                <Script src={`/javascript/${frontmatter.type}/${frontmatter.assets["main-js"]}`} />
            )}
...
export async function getStaticProps({ params: { slug } }) {
    ...
    const cmpStyles = frontmatter.assets["main-css"]
        ? fs.readFileSync(path.join("public/css", frontmatter.type, frontmatter.assets["main-css"]), "utf-8")
        : null;
    return {
        props: {
            frontmatter,
