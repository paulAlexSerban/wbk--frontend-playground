...
export const UIController = (el) => {
        ...
        if ('question-'.indexOf(event.target.id)) {
          ...
          <input type="text" class="admin-option admin-option-${x}" value="${foundItem.options[x]}">
            ...
            const backDefaultView = () => {
                ...
                const updatedOptions = document.querySelectorAll('.admin-option');
            ...
            const updateQuestion = function () {
                const newOptions = [];
                const optionEls = document.querySelectorAll('.admin-option');
    ...
    const setupDisplayQuestion = (storageQuestList, progress) => {
        let newOptionHTML;
        const characterArr = ['A', 'B', 'C', 'D', 'E', 'F'];
        if (storageQuestList.getQuestionCollection().length > 0) {
    ...
    const setupDeleteResult = (event, userData) => {
        let getId;
        const personArr = userData.getPersonData();
