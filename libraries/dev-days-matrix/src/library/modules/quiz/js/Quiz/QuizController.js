export const QuizController = () => {
    ...
    const setupQuestionLocalStorage = (newQuestText, opts) => {
        ...
        if (questionLocalStorage.getQuestionCollection().length > 0) {
            questionId =
                questionLocalStorage.getQuestionCollection()[questionLocalStorage.getQuestionCollection().length - 1]
                    .id + 1;
        } else {
            questionId = 0;
        }
    ...
    const setupCheckAnswer = (ans) => {
        if (
            questionLocalStorage.getQuestionCollection()[quizProgress.questionIndex].correctAnswer === ans.textContent
        ) {
    ...
    const setupAddPerson = () => {
        ...
        const personData = personLocalStorage.getPersonData();
