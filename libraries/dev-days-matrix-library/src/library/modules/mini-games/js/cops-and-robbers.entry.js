...
let police = {};
let thug = {};
const playerOneNameInput = $('.player__name-input--one');
const playerTwoNameInput = $('.player__name-input--two');

const weapons = ['weapons__multitool-knife', 'weapons__baton', 'weapons__gun', 'weapons__rifle'];
...
class Player {
    ...
    adjacentCheck(nextCell) {
        const nextCellRow = parseInt(nextCell.attr('data-row'));
        const nextCellCol = parseInt(nextCell.attr('data-col'));

        const topCell = $(`.cell__${nextCellRow + -1}-${nextCellCol}`);
        const rightCell = $(`.cell__${nextCellRow}-${nextCellCol + 1}`);
        const bottomCell = $(`.cell__${nextCellRow + 1}-${nextCellCol}`);
        const leftCell = $(`.cell__${nextCellRow}-${nextCellCol + -1}`);
    ...
    moveDirection(vertical, horizontal) {
        const oldCell = $(`.cell__${this.positionRow}-${this.positionCol}`);
        const nextCell = $(`.cell__${this.positionRow + vertical}-${this.positionCol + horizontal}`);
            ...
            for (let i = 0; i < weapons.length; i++) {
                if (nextCell.hasClass(weapons[i]) === true) {
                    if (this.collectedWeapon === 'none') {
                        ...
                        const w = weapons.indexOf(this.oldWeapon);
    ...
    continueAttack() {
        if (this.health <= 0) {
            const enemy = eval(this.enemyRole);
...
function createNewPlayers() {
    const policeInitPositionRow = parseInt($('.player__policeAvatar')[0].dataset.row);
    const policeInitPositionCol = parseInt($('.player__policeAvatar')[0].dataset.col);
    const thugInitPositionRow = parseInt($('.player__thugAvatar')[0].dataset.row);
    const thugInitPositionCol = parseInt($('.player__thugAvatar')[0].dataset.col);
    police = new Player(
        ...
        if (ev.which === 38) {
            ...
            if (cellsUp > 3) {
                ...
                for (let i = 1; i <= cellsUp; i++) {
                    const oldCell = $(`.cell__${eval(role).positionRow}-${eval(role).positionCol}`);
                    const nextCell = $(`.cell__${eval(role).positionRow + -1}-${eval(role).positionCol}`);
                    const nextCellRow = parseInt(nextCell.attr('data-row'));
                    const nextCellCol = parseInt(nextCell.attr('data-col'));
                    const topCell = $(`.cell__${nextCellRow + -1}-${nextCellCol}`);
                    const rightCell = $(`.cell__${nextCellRow}-${nextCellCol + 1}`);
            ...
            if (cellsRight > 3) {
                ...
                for (let i = 1; i <= cellsRight; i++) {
                    const oldCell = $(`.cell__${eval(role).positionRow}-${eval(role).positionCol}`);
                    const nextCell = $(`.cell__${eval(role).positionRow}-${eval(role).positionCol + 1}`);
                    const nextCellRow = parseInt(nextCell.attr('data-row'));
                    const nextCellCol = parseInt(nextCell.attr('data-col'));
                    const topCell = $(`.cell__${nextCellRow + -1}-${nextCellCol}`);
                    const rightCell = $(`.cell__${nextCellRow}-${nextCellCol + 1}`);
...
function cellPositioning(grid) {
    ...
    const squaresArray = gridContainer.children().toArray();
    const players = ['player__policeAvatar', 'player__thugAvatar'];

    function randomIndexGenerator(side, size, fraction) {
        const randomIndex = side * size - Math.floor(Math.random() * (size * fraction));
        return randomIndex;
    }
    let pickedCell;
