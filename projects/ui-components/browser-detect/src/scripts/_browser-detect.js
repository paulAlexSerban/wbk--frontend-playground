...
const resolveBrowserInfo = (ua) => {
    ...
    if (chromeMatch) {
        return `Chrome version ${chromeMatch[1]} on ${osString}`;
    }
    if (criosMatch) {
        return `Chrome version ${criosMatch[1]} (with CriOS indicating it is Chrome for iOS) on ${osString}`;
    }
    if (firefoxMatch) {
        return `Firefox version ${firefoxMatch[1]} on ${osString}`;
    }
    if (gsaMatch) {
        return `Google Search App version ${gsaMatch[1]} on ${osString}`;
    }
    if (edgeMatch) {
        return `Edge version ${edgeMatch[1]} on ${osString}`;
    }
...
export const initPattern = () => {
    ...
    document.querySelectorAll('[data-js-cmp="BrowserDetect"]').forEach((root) => {
